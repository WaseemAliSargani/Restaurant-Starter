  ...
  return <section className='min-h-[780px]'>
    ...
    <div className='relative z-10 top-72 lg:top-52'>
      ...
      className='container mx-auto flex flex-col items-center text-center'>
        ...
        className='h2 capitalize text-white max-w-[400px]'> {title} </motion.h2>
        <motion.p 
            ...
            {menuItems.map((item, index) => {
              ...
              return <div key={index}>
                <div className='flex flex-row lg:flex-col h-full'>
                  ...
                  {/* image */}
                  {/* reverse order for the first image one hte last one */}
                  <div
                    className={`w-[45%] md:w-auto ${index === 1 || index === 3 ? 'lg:order-1' : 'order-none'} `}
                  >
                    <img src={image} alt="" />
                  </div>

                  {/* text */}
                  <div className='bg-[#fff3e4] flex-1 flex flex-col justify-center px-6 lg:p-12  lg:max-h-[250px] xl:max-h-[293px]'>
                    <div className='text-center'>
                      <div className='text-xl font-semibold text-dark xl:text-2xl '> {name} </div>
                      <div className='my-1 text-[20px] md:text-[30px] lg:text-[40px] md:my-4 lg:my-6 text-accent font-semibold'> {price} </div>
                      <div> {description} </div>
                    </div>
                  </div>
                </div>
